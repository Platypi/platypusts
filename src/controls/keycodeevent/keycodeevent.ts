module plat.controls {
    ...
    export class KeyCodeEventControl extends SimpleEventControl implements IKeyCodeEventControl {
        ...
        protected _setListener(): void {
            ...
            if (expression[0] === '{') {
                let eventObject: IKeyboardEventInput = this.evaluateExpression(expression);
        ...
        protected _filterArgs(input: IKeyboardEventInput): any[] {
            ...
            if (isArray(keys)) {
                ...
                if (!(isNull(input.char) && isNull(input.chars))) {
                    this._log.warn(`${this.type} should be using the property key or keys to denote key codes or keys and not char codes or characters.`);
                }
            }
        }
    ...
    export class KeyPress extends KeyCodeEventControl {
        ...
        public _onEvent(ev: KeyboardEvent): void {
            ...
            if (super._compareKeys(ev) && ((keyCode >= 48 && keyCode <= 90) ||
                ...
                const remove = this.addEventListener(this.element, 'keypress', (e: KeyboardEvent): void => {
                    remove();
                    super._onEvent(e);
                }, false);
            }
